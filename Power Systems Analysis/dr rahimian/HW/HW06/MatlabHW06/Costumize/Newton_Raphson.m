...
Y_bus_Amplitude=zeros(nbuses,nbuses);
Y_bus_Phase=zeros(nbuses,nbuses);
for i=1:length(Y(:,1))
    for j=1:length(Y(1,:))
        Y_bus_Amplitude(i,j)=abs(Y(i,j));
        Y_bus_Phase(i,j)=angle(Y(i,j))*180/pi;
    end
end
