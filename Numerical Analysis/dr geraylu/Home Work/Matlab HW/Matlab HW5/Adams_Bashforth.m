clear;clc;close all;

menu = input('Enter one of them:\n 1) 2.step Adams Bashforth (AB2)\n 2) 4.step Adams Bashforth (AB4)\n ====> ');

f = input('Enter equation to be solved in terms of x,y (@(x,y)=... ): ');
a = input('Enter low limit period: ');
b = input('Enter high limit period: ');
h = input('Enter the step size (h): ');
x = a:h:b;
y = zeros(1,length(x));
y(1) = input('Enter the intial value for y ( y(0) ): ');
disp('--------------------------------------');

if (menu == 1)
    for i = 1:length(x)-1
        k1 = h * f(x(i), y(i));
        k2 = h * f(x(i) + h, y(i) + k1);
        y(i+1) = y(i) + 0.5*(k1 + k2);
    end
    fprintf('2.Step Adams Bashforth (AB2): \n');
    fprintf('k      x_k        y_k \n');
    fprintf('%d)    %2.3f     %f \n', 0, x(1), y(1));
    fprintf('%d)    %2.3f     %f \n', 1, x(2), y(2));
    for i=3:length(x)
        y(i+1) = y(i) + (h/12)*( 23*f(x(i), y(i) ) - 16*f( x(i-1), y(i-1) ) + 5*f( x(i-2), y(i-2) ));
        fprintf('%d)    %2.3f     %f \n', i, x(i), y(i))
    end   
end

if (menu == 2)
   for i = 1:length(x)-1
        k1 = h * f(x(i), y(i));
        k2 = h * f(x(i) + 0.5*h, y(i) + 0.5*k1);
        k3 = h * f(x(i) + 0.5*h, y(i) + 0.5*k2);
        k4 = h * f(x(i) + h, y(i) + k3);
        y(i+1) = y(i) + 1/6 *(k1 + 2*k2 + 2*k3 +k4);
   end
   fprintf('4.Step Adams Bashforth (AB4): \n');
   fprintf('k      x_k        y_k \n');
   fprintf('%d)    %2.3f     %f \n', 0, x(1), y(1));
   fprintf('%d)    %2.3f     %f \n', 1, x(2), y(2));
   fprintf('%d)    %2.3f     %f \n', 2, x(3), y(3));
   fprintf('%d)    %2.3f     %f \n', 3, x(4), y(4));
   for i=4:length(x)
        y(i+1) = y(i) + (h/24)*( 55*f(x(i), y(i) ) - 59*f( x(i-1), y(i-1) ) + 37*f( x(i-2), y(i-2) ) ...
            -9*f( x(i-3), y(i-3) ) );
        fprintf('%d)    %2.3f     %f \n', i, x(i), y(i))
   end  
end
