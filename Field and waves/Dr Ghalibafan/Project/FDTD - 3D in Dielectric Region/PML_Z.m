function[aez,bez,amz,bmz,CPsi_eyz,CPsi_exz,CPsi_hxz,CPsi_hyz,...
    ...
    % ADE Conditions
    rho_e    = [((PnZ:-1:1)-0.25)/PnZ,((1:PnZ)-0.25)/PnZ];
    rho_m    = [((PnZ:-1:1)+0.25)/PnZ,((1:PnZ)+0.25)/PnZ];
    ...
    CPsi_eyz = Ceyhx(:,:,[2:PnZ+1,Nz-PnZ+1:Nz])*dz;
    CPsi_exz = Cexhy(:,:,[2:PnZ+1,Nz-PnZ+1:Nz])*dz;
    Ceyhx(:,:,[2:PnZ+1,Nz-PnZ+1:Nz]) = Ceyhx(:,:,[2:PnZ+1,Nz-PnZ+1:Nz])./kappa_e;
    Cexhy(:,:,[2:PnZ+1,Nz-PnZ+1:Nz]) = Cexhy(:,:,[2:PnZ+1,Nz-PnZ+1:Nz])./kappa_e;
    CPsi_hyz = Chyex*dz;
    CPsi_hxz = Chxey*dz;
    Chxey = Chxey.*ones(1,1,Nz);
    Chyex = Chyex.*ones(1,1,Nz);
    Chyex(:,:,[1:PnZ,Nz-PnZ+1:Nz]) = Chyex(:,:,[1:PnZ,Nz-PnZ+1:Nz])./kappa_m;
    Chxey(:,:,[1:PnZ,Nz-PnZ+1:Nz]) = Chxey(:,:,[1:PnZ,Nz-PnZ+1:Nz])./kappa_m;
    Cexhy = Cexhy(:,2:Ny,2:Nz);
    Ceyhx = Ceyhx(2:Nx,:,2:Nz);
